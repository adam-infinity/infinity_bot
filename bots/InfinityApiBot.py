...
class InfinityApiBot:
    ...
    def __init__(self, verify, send_orders=True, cancel_orders=True, update_active_orders=True):
        ...
        self.floating_markets, self.floating_tokens_and_prices =\
            Inf.list_floating_rate_markets(self.domain, self.verify)
    ...
    def cancel_fixed_order(self, order_id):
        if self.cancel_orders:
            if order_id not in self.cancelled_fixed_orders:
                ...
                Inf.cancel_fixed_order(order_id, self.domain, self.cookies, self.verify)
    ...
    def cancel_floating_order(self, order_id):
        if self.cancel_orders:
            if order_id not in self.cancelled_floating_orders:
                ...
                Inf.cancel_floating_order(order_id, self.domain, self.cookies, self.verify)
    ...
    def get_fixed_rate_orders(self, token_id, days_to_maturity, wallet_id=None):
    ...
    def get_floating_rate_market_history(self, floating_market_id):
        return Inf.get_floating_rate_market_history(floating_market_id, self.domain, self.verify)

    def get_recent_fixed_rate_market_transactions(self, fixed_rate_market_id):
        return Inf.get_recent_fixed_rate_market_transactions(fixed_rate_market_id,  self.domain, self.verify)
    ...
    def list_all_fixed_rate_markets(self):
        ...
        fixed_rate_markets = {}
        for token_id in self.floating_tokens_and_prices:
            fixed_rate_markets[token_id] = Inf.list_fixed_rate_markets(self.domain, token_id, self.verify)
    ...
    def update_last_prices(self):
        self.floating_tokens_and_prices = Inf.list_tokens(self.domain, self.verify)
